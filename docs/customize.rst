...
The keypresses in the action will be interpreted relative to the keyset you are currently using (original, original with hjkl movement, Angband-like, or Angband-like with hjkl movement).  The game will remember what keyset was in effect when the keymap was created.  So if you change keysets, the keymaps which were only defined for the other keyset won't be visible.  You can have more than one keymap, up to one for each keyset, bound to the same trigger.
...
The action must always come first,  ```<type>``` means 'keyset type', which is either 0 for the original keyset, 1 for the original keyset with hjkl movement, 2 for the Angband-like keyset, or 3 for the Angband-like keyset with hjkl movement.  For example::

	keymap-act:Ea
	keymap-input:0:[F1]

An action can have more than one trigger bound to it by having more than
one keymap-input line after it and before the next keymap-act line.  The
reason to do that would be to have the keymap work with more than one keyset.
For example::
