...
static void spoil_obj_gen(const char *fname)
{
	...
	file_putf(fff,
			  "Object Generation Spoiler for Beleriand Version %d.%d.%d\n",
			  VERSION_MAJOR, VERSION_MINOR, VERSION_PATCH);
...
static void spoil_mon_gen(const char *fname)
{
	...
	file_putf(fff,
			  "Monster Generation Spoiler for Beleriand Version %d.%d.%d\n",
			  VERSION_MAJOR, VERSION_MINOR, VERSION_PATCH);
	...
	for (i = 0L; i < 1000000L; i++) {
		...
		/* Get a monster index */
		if (quick)
			num = get_mon_num_quick(p_ptr->danger);
		else
			num = get_mon_num(p_ptr->danger);
