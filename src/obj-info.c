						   ...
						   oinfo_detail_t mode)
{
	...
	if (count > 0) {
		byte attr =
			(o_ptr->bonus_other[0] > 0 ? TERM_L_GREEN : TERM_ORANGE);
		...
		for (j = 0; j < MAX_P_BONUS; j++) {
			if (o_ptr->bonus_other[j] == 0)
				continue;
			attr =
				(o_ptr->bonus_other[j] > 0 ? TERM_L_GREEN : TERM_ORANGE);
							...
							oinfo_detail_t mode)
{
	...
	if (of_has(o_ptr->flags_obj, OF_THROWING)) {
		if (of_has(o_ptr->id_obj, OF_PERFECT_BALANCE)) {
			if (terse)
				textblock_append(tb, "Well-balanced.\n");
			else
				textblock_append(tb, "It can be thrown hard and fast.\n");
		} else {
			if (terse)
				textblock_append(tb, "Throwable.\n");
			else
				textblock_append(tb, "It can be thrown effectively.\n");
		}
		return TRUE;
	}
							  ...
							  oinfo_detail_t mode)
{
	...
	if (count == 6)
		textblock_append(tb, "It sustains all your stats. ");
	else {
	...
	for (i = 0; i < 4; i++) {
		...
		if (i == 0 && chance > 0) {
			if (sl == INVEN_WIELD)
				...
				textblock_append(tb,
								 "With this item, your current weapon clears ");
		}
		...
		if (chance == 1600) {
			...
			textblock_append_c(tb, TERM_GREEN, "%d.%d ", decis / 10,
							   decis % 10);
		} else {
								  ...
								  oinfo_detail_t mode)
{
	...
	if (subjective)
		(void) describe_origin(tb, o_ptr);
	if (!terse)
		(void) describe_flavor_text(tb, o_ptr);
 ...
 */
void object_info_chardump(const object_type * o_ptr,
						  char_attr_line ** line, int *current_line,
						  int indent, int wrap)
{
