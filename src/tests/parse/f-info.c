...
static int test_flags0(void *state) {
	...
	r = parser_parse(p, "flags:PERMANENT | DOWNSTAIR");
	eq(r, PARSE_ERROR_NONE);
	f = (struct feature*) parser_priv(p);
	notnull(f);
...
static int test_dig_msg0(void *state) {
	struct parser *p = (struct parser*) state;
	enum parser_error r = parser_parse(p, "dig-msg:You clear the rubble.");
	struct feature *f;

	eq(r, PARSE_ERROR_NONE);
	/* Check that additional directives are appended to the first. */
	r = parser_parse(p, "dig-msg:  Revealing a dusty floor.");
	eq(r, PARSE_ERROR_NONE);
	f = (struct feature*) parser_priv(p);
	notnull(f);
	notnull(f->dig_msg);
	require(streq(f->dig_msg, "You clear the rubble.  Revealing a dusty floor."));
	ok;
}
...
static int test_fail_msg0(void *state) {
	...
	eq(r, PARSE_ERROR_NONE);
	/* Check that additional directives are appended to the first. */
	r = parser_parse(p, "fail-msg:  And develop a blister on your palm.");
	eq(r, PARSE_ERROR_NONE);
	f = (struct feature*) parser_priv(p);
	notnull(f);
	notnull(f->fail_msg);
	require(streq(f->fail_msg, "You are unable to shift the rubble with "
		"your {name}.  And develop a blister on your palm."));
	ok;
}
...
static int test_str_msg0(void *state) {
	...
	eq(r, PARSE_ERROR_NONE);
	/* Check that additional directives are appended to the first. */
	r = parser_parse(p, "str-msg:  Eat more spinach next time.");
	eq(r, PARSE_ERROR_NONE);
	f = (struct feature*) parser_priv(p);
	notnull(f);
	notnull(f->str_msg);
	require(streq(f->str_msg,
		"You are not strong enough to shift the rubble.  Eat more "
		"spinach next time."));
	ok;
}
