	...
	for (y = 0; y < CHUNK_HGT; y++) {
		for (x = 0; x < CHUNK_WID; x++) {
			...
			if (cave_m_idx[y0 + y][x0 + x] > 0) {
				...
				if ((m_ptr->y_terr >= y0)
					&& (m_ptr->y_terr < y0 + CHUNK_HGT)
					&& (m_ptr->x_terr >= x0)
					&& (m_ptr->x_terr < x0 + CHUNK_WID)) {
...
int chunk_find(chunk_ref ref)
{
	...
	for (i = 0; i < MAX_CHUNKS; i++) {
		/* Reject wrong values */
		if (ref.region != chunk_list[i].region)
			continue;
		if (ref.x_pos != chunk_list[i].x_pos)
			continue;
		if (ref.y_pos != chunk_list[i].y_pos)
			continue;
		if (ref.z_pos != chunk_list[i].z_pos)
			continue;
	...
	for (y = 0; y < CHUNK_HGT; y++) {
		for (x = 0; x < CHUNK_WID; x++) {
			...
			if (chunk->cave_m_idx[y][x] > 0) {
				monster_type *m_ptr =
					&chunk->m_list[chunk->cave_m_idx[y][x]];
				monster_type *n_ptr = NULL;
				int m_idx;
 ...
 */
void chunk_adjacent_to_offset(int adjacent, int *z_off, int *y_off,
							  int *x_off)
{
	if (adjacent == DIR_UP) {
		*z_off = -1;
...
int chunk_get_idx(int z_offset, int y_offset, int x_offset)
{
	...
	if (adj_index == -1)
		quit_fmt("No chunk at y offset %d, x offset %d", y_offset,
				 x_offset);
...
int find_region(int y_pos, int x_pos)
{
	...
	for (i = 0; i < z_info->region_max; i++) {
		...
		entry =
			region->width * ((y_pos / 10) - region->y_offset) + x_pos / 10;
		assert(entry >= 0);
 ...
 */
void edge_add(edge_effect ** first, edge_effect ** latest,
			  edge_effect ** current)
{
	if (*first) {
		(*latest)->next = *current;
...
void chunk_generate(chunk_ref ref, int y_offset, int x_offset)
{
	...
	for (n = 0; n < DIR_MAX; n++) {
		...
		if ((x_off == 0) || (y_off == 0)) {
			...
			if (exists) {
				...
				case DIR_UP:
					{
						for (start = loc->effect; start->next;
							 start = start->next) {
							current = &vertical[start->y][start->x];

							if (tf_has
								(f_info[start->terrain].flags,
								 TF_DOWNSTAIR)
								|| tf_has(f_info[start->terrain].flags,
										  TF_FALL)) {
	...
	for (n = 0; n < z_info->l_max; n++) {
		...
		if (l_ptr->map_z != z_pos)
			continue;
		if (l_ptr->map_y > y_pos)
			continue;
		if (l_ptr->map_y + l_ptr->height <= y_pos)
			continue;
		if (l_ptr->map_x > x_pos)
			continue;
		if (l_ptr->map_x + l_ptr->width <= x_pos)
			continue;
