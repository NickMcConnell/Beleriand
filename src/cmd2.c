...
void do_cmd_go_up(cmd_code code, cmd_arg args[])
{
	...
	if (p_ptr->themed_level)
		if (!get_check
			("This level will never appear again.  Really leave?"))
			return;
...
static void chest_trap(int y, int x, s16b o_idx)
{
	...
	if (trap & (CHEST_SCATTER)) {
		if (chunk_list[p_ptr->stage].z_pos > 0)
			msg("The contents of the chest scatter all over the dungeon!");
		else
	...
	if (trap & (CHEST_BIRD_STORM)) {
		...
		j = randint1(3) + 3;
		for (i = 0; i < j; i++)
			(void) fire_meteor(0, GF_FORCE, y, x, o_ptr->pval / 5, 7,
							   TRUE);
