		   ...
		   bool notice_launcher, bool notice_ring)
{
	    ...
	    && (rf_has(r_ptr->flags, slays[i].slain))) 
	{
	    ...
	    if (*mul < slay[i])	*mul = slay[i];

	    /* Special case for holy attacks */
	    if ((i == P_SLAY_EVIL) && (p_ptr->special_attack & (ATTACK_HOLY)) 
		&& (*mul < 15))
		*mul = 15;
		      ...
		      monster_type *m_ptr, int item)
{
    ...
    case TV_DIGGING:
    {
	get_slay_info(m_ptr, slay, &mul, item, notice_launcher, notice_ring);
	get_brand_info(m_ptr, brand, &mul, item, notice_launcher, notice_ring);
			    ...
			    int power, int deadliness)
{
	...
	if (power_strike)
	    msgt(MSG_HIT, "Power Strike! You attempt to confuse %s.", m_name);
	else
 ...
 */
bool attempt_shield_bash(int y, int x, bool *fear, int *blows, char *m_name)
{
...
bool py_attack(int y, int x, bool can_push)
{
	...
	if (test_hit_combat
		...
		/* Calculate criticals. */
		crit_dice =
		    critical_melee(chance, sleeping_bonus, m_ptr->ml, m_name,
				   o_ptr);
		...
		sides =
		    (temp / 10000) + (randint0(10000) < (temp % 10000) ? 1 : 0);
...
void do_cmd_fire(cmd_code code, cmd_arg args[])
{
	...
	if (cave_m_idx[y][x] > 0) {
	    ...
	    if (!
		(test_hit_combat
		 (chance2 + sleeping_bonus, armour, m_ptr->ml, INVEN_BOW + 1,
		  ((item < 0) ? item : item + 1)))) {
		did_miss = TRUE;
	    ...
	    /* Critical hits may add damage dice. */
	    dice +=
		critical_shot(chance2, sleeping_bonus, FALSE, m_ptr->ml, m_name,
			      i_ptr);
	    ...
	    /* Calculate the actual number of sides to each die. */
	    sides = (temp / 10000) + (randint0(10000) < (temp % 10000) ? 1 : 0);
...
void do_cmd_throw(cmd_code code, cmd_arg args[])
{
	...
	if (cave_m_idx[y][x] > 0) {
		...
		if (m_ptr->ml) {
		    msgt(MSG_HIT, "The %s hits %s.", o_name,
				   m_name);
		} else {
		    msgt(MSG_HIT, "The %s finds a mark.", o_name);
		}
