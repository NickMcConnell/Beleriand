 ...
 */
static void spread_objects(int depth, int num, int y0, int x0, int dy,
						   int dx)
{
	...
	if (randint0(36) == 0) {
		...
		/* The ceiling has collapsed. */
			...
			if ((p_ptr->danger > 35) && (randint0(3) == 0)) {
				spread_monsters('X', p_ptr->danger, 2 + randint1(3), y0,
								x0, 3, 9);
		...
		/* A lake and/or a forest now fills the room. */
			...
			if (generate_starburst_room
				...
				if ((p_ptr->danger > 15) && (randint0(4) == 0)) {
					spread_monsters('6', p_ptr->danger, 2 + randint1(4),
									y0, x0, 3, 7);
			...
			if ((!water_room) && (randint0(2) == 0)) {
				...
				/* Animals love trees. */
				if (randint0(6) == 0)
					spread_monsters('3', p_ptr->danger, 2 + randint0(6),
									y0, x0, 4, 11);
				else if (randint0(3) == 0)
					spread_monsters('A', p_ptr->danger, 2 + randint0(5),
									y0, x0, 4, 11);
			}
		}
	}
...
static bool build_type2(void)
{
	...
	if ((p_ptr->danger >= 12) && (randint0(40) == 0)) {
		...
		if (randint0(3) == 0) {
			...
			if (p_ptr->danger > 45)
				spread_monsters('U', p_ptr->danger, 3 + randint0(5), y0,
								x0, height / 2, width / 2);
			else
				spread_monsters('u', p_ptr->danger, 2 + randint0(3), y0,
								x0, height / 2, width / 2);
		}
	...
	case 2:
		{
			...
			if (randint0(3) == 0) {
				/* Inner rectangle */
				generate_draw(y0 - 1, x0 - 5, y0 + 1, x0 + 5,
							  FEAT_WALL_INNER);
				...
				/* Monsters */
				spread_monsters('\0', p_ptr->danger, randint1(4), y0, x0,
								2, 7);
	...
	case 3:
		{
			...
			if (randint0(3) == 0)
				spread_monsters('N', p_ptr->danger, randint1(6), y0, x0, 2,
								9);
			else if (randint0(3) == 0)
				spread_monsters('*', p_ptr->danger, randint1(6), y0, x0, 2,
								9);
			else
				spread_monsters('\0', p_ptr->danger, randint1(6), y0, x0,
								2, 9);
	...
	case 4:
		{
			...
			spread_monsters('\0', p_ptr->danger, 6 + randint0(11), y0, x0,
							2, 9);
...
static bool build_type5(void)
{
	...
	depth =
		p_ptr->danger + 3 + (p_ptr->danger < 70 ? p_ptr->danger / 7 : 10);
 ...
 */
extern bool build_vault(int y0, int x0, int ymax, int xmax,
						const char *data, bool light, bool icky,
						byte vault_type)
{
	int x, y;
	int y1, x1, y2, x2;
...
static bool build_type10(void)
{
	...
	if (p_ptr->danger > 40) {
		...
		if (!generate_starburst_room
			(y1, x1, y2, x2, light, FEAT_FLOOR, FALSE))
			return (FALSE);
		...
		if (randint1(5) > 2) {
			...
			for (i = 0; i < count; i++) {
				...
				generate_starburst_room(y1_tmp, x1_tmp, y2_tmp, x2_tmp,
