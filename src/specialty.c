...
static void gain_spec_menu_browser(int oid, void *data, const region * loc)
{
	...
	text_out_to_screen(TERM_DEEP_L_BLUE,
					   (char *) abilities[d->specialties[oid]].desc);
...
void gain_specialty(void)
{
	...
	if (gain_spec_menu(&pick)) {
		...
		/* Specialty taken */
		sprintf(buf, "Gained the %s specialty.", abilities[pick].name);
...
static void view_abilities_aux(char *desc)
{
	...
	if ((of_has(flags, OF_SUSTAIN_STR)) || (of_has(flags, OF_SUSTAIN_INT))
		|| (of_has(flags, OF_SUSTAIN_WIS))
		|| (of_has(flags, OF_SUSTAIN_DEX))
		|| (of_has(flags, OF_SUSTAIN_CON))
		|| (of_has(flags, OF_SUSTAIN_CHR))) {
					   ...
					   int col, int width)
{
	...
	/* Print it */
	c_put_str(cursor ? get_color(color, ATTR_HIGH, 1) : color, buf, row,
			  col);
