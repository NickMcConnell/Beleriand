...
void sort_quiver(void)
{
	...
	for (i = 0; i < QUIVER_SIZE; i++) {
		...
		if (p_ptr->inventory[QUIVER_START + i].k_idx)
			continue;

		/* Start from the end and find an unlocked item to put here. */
		for (j = QUIVER_SIZE - 1; j > i; j--) {
			if (!p_ptr->inventory[QUIVER_START + j].k_idx || locked[j])
				continue;
...
void open_quiver_slot(int slot)
{
	...
	/* Quiver is full */
	if (p_ptr->inventory[QUIVER_END - 1].k_idx)
		return;

	/* Find the first open quiver slot */
	while (p_ptr->inventory[dest].k_idx)
		dest++;
...
bool inven_stack_okay(const object_type * o_ptr)
{
	...
	for (j = 0; j < limit; j++) {
		...
		if (j >= INVEN_PACK && j < QUIVER_START)
			continue;
		if (!j_ptr->k_idx)
			continue;
 ...
 */ static int compare_items(const object_type * o1,
...
void display_itemlist(void)
{
		...
		for (mx = 0; mx < dungeon_wid; mx++) {
			...
			for (i = 0; i < num; i++) {
				...
				if (j == counter) {
					...
					while (j > 0
						   && compare_items(types[j - 1], types[j]) > 0) {
	...
	for (i = 0; i < counter; i++) {
		...
		object_desc(o_name, sizeof(o_name), o_ptr, ODESC_FULL);
		if (counts[i] > 1)
			strnfmt(o_desc, sizeof(o_desc), "%s (x%d) %d %c, %d %c",
					o_name, counts[i], (dy[i] > 0) ? dy[i] : -dy[i],
		...
		if (Term == angband_term[0] && (line == max)
			&& disp_count != counter) {
			prt("-- more --", line, x);
			anykey();
			...
			/* Reprint Message */
			if (counter == MAX_ITEMLIST) {
				prt(format("You can see many items:"), 0, 0);
			} else {
				prt(format
					("You can see %d item%s:", counter,
					 (counter > 1 ? "s" : "")), 0, 0);
			}
			/* Reset */
			line = 1;
...
int scan_items(int *item_list, size_t item_list_max, int mode)
{
	...
	if (use_floor) {
		floor_num =
			scan_floor(floor_list, N_ELEMENTS(floor_list), p_ptr->py,
					   p_ptr->px, 0x03);
 ...
 */
bool item_is_available(int item, bool(*tester) (const object_type *),
					   int mode)
{
				...
				for (j = 0; j < (set_ptr->no_of_items); j++) {
					...
					if (se_ptr->a_idx == o_ptr->name1) {
						...
						if (!(a_ptr->set_bonus)) {
							...
							cf_union(o_ptr->flags_curse,
									 se_ptr->flags_curse);
							for (k = 0; k < MAX_P_RES; k++)
								if (se_ptr->percent_res[k] !=
									RES_LEVEL_BASE)
									o_ptr->percent_res[k] =
