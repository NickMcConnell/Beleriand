	...
	case SHAPE_WYRM:
		{
			...
			case (SV_DRAGON_BLACK):
				{
					apply_resist(&player_resists[P_RES_ACID],
								 RES_BOOST_IMMUNE);
					break;
				}
			case (SV_DRAGON_BLUE):
				{
					apply_resist(&player_resists[P_RES_ELEC],
								 RES_BOOST_IMMUNE);
					break;
				}
			case (SV_DRAGON_WHITE):
				{
					apply_resist(&player_resists[P_RES_COLD],
								 RES_BOOST_IMMUNE);
					break;
				}
			case (SV_DRAGON_RED):
				{
					apply_resist(&player_resists[P_RES_FIRE],
								 RES_BOOST_IMMUNE);
					break;
				}
			}
 ...
 */
static void prt_deci(const char *header, int num, int deci, int row,
					 int col, byte color)
{
	int len = strlen(header);
	char out_val[32];
...
void display_player_sml(void)
{
	...
	prt_num("Blows/Round      ", p_ptr->state.num_blow, 15, 1,
			TERM_L_BLUE);
	...
	prt_deci("Shots/Round   ", p_ptr->state.num_fire / 10,
			 p_ptr->state.num_fire % 10, 15, 26, TERM_L_BLUE);
...
extern int make_dump(char_attr_line * line, int mode)
{
	...
	if (mode != 1) {
		...
		if (p_ptr->lev >= p_ptr->max_lev)
			dump_num("Level            ", (int) p_ptr->lev, 27,
					 TERM_L_GREEN);
		else
			dump_num("Level            ", (int) p_ptr->lev, 27,
					 TERM_YELLOW);
		...
		if (p_ptr->lev >= PY_MAX_LEVEL) {
			...
			dump_lnum("Exp to Adv.   ",
					  (s32b) (player_exp[p_ptr->lev - 1]), 27,
					  TERM_L_GREEN);
		...
		if (!p_ptr->inventory[INVEN_WIELD].k_idx) {
			...
			if (show_m_todam >= 0)
				dump_num("Deadliness (%)   ",
						 deadliness_conversion[show_m_todam], 1,
						 TERM_L_BLUE);
			else
				dump_num("Deadliness (%)   ",
						 -deadliness_conversion[-show_m_todam], 1,
		...
		if (show_a_todam > 0)
			dump_num("Deadliness (%)   ",
					 deadliness_conversion[show_a_todam], 53, TERM_L_BLUE);
		else
			dump_num("Deadliness (%)   ",
					 -deadliness_conversion[-show_a_todam], 53,
					 TERM_L_BLUE);
		...
		current_line++;
		dump_ptr = (char_attr *) & line[current_line];
		text_out_dump(TERM_WHITE, p_ptr->history, &line, &current_line, 10,
					  60);
	...
	for (y = 0; y < 7; y++) {
		...
		for (x = 0; x < 2; x++) {
			...
			/* Name */
			dump_put_str(resist_colour
						 (p_ptr->state.dis_res_list[y + x * 7]), name1,
						 1 + 24 * x);

			/* Check equipment */
			for (n = 6 + 25 * x, i = INVEN_WIELD; i < INVEN_TOTAL;
				 i++, n++) {
				object_type *o_ptr;
			...
			/* Percentage */
			sprintf(buf1, "%4d%%",
					100 - p_ptr->state.dis_res_list[y + x * 7]);
			dump_put_str(resist_colour
						 (p_ptr->state.dis_res_list[y + x * 7]), buf1,
						 n + 1);
		}
		current_line++;
	}
	...
	if (p_ptr->equip_cnt) {
		...
		for (i = INVEN_WIELD; i < ALL_INVEN_TOTAL; i++) {
			if (i == INVEN_TOTAL) {
				...
				dump_put_str(proc_list_color_hack(&p_ptr->inventory[i]),
							 buf, 0);
				current_line++;
				dump_ptr = (char_attr *) & line[current_line];
				object_info_chardump(&p_ptr->inventory[i], &line,
									 &current_line, 5, 75);
				current_line++;
	...
	for (i = 0; i < INVEN_PACK; i++) {
		...
		current_line++;
		dump_ptr = (char_attr *) & line[current_line];
		object_info_chardump(&p_ptr->inventory[i], &line, &current_line, 5,
							 75);
		current_line++;
	...
	dump_ptr = (char_attr *) & line[current_line];
	sprintf(buf, "%s the %s %s", op_ptr->full_name, rp_ptr->name,
			cp_ptr->name);
	dump_put_str(TERM_WHITE, buf, 0);
	current_line++;
	dump_ptr = (char_attr *) & line[current_line];
...
void display_dump(char_attr_line * line, int top_line, int bottom_line,
				  int col)
{
	int i;
...
bool show_file(const char *name, const char *what, int line, int mode)
{
	...
	while (TRUE) {
		...
		/* Prompt -- small files */
			...
			/* Wait for it */
			strncpy(prompt, "[Press ESC for previous file, ? to exit.]",
					80);
		}
...
static void write_html_escape_char(ang_file * htm, wchar_t c)
{
	...
	default:
		{
			char *mbseq =
				(char *) mem_alloc(sizeof(char) * (MB_CUR_MAX + 1));
			byte len;
			len = wctomb(mbseq, c);
			if (len > MB_CUR_MAX)
				len = MB_CUR_MAX;
