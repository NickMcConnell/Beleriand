...
static void py_pickup_gold(void)
{
	...
	if (total_gold) {
		...
		if (total_gold < 200)
			sound_msg = MSG_MONEY1;
		else if (total_gold < 600)
			sound_msg = MSG_MONEY2;
		else
			sound_msg = MSG_MONEY3;
...
extern void py_pickup_aux(int o_idx, bool domsg)
{
	...
	if ((o_ptr->feel == FEEL_AVERAGE)
		&& (is_weapon(o_ptr) || is_armour(o_ptr))) {
	...
	if (quiver_slot)
		wield_item(o_ptr, slot, quiver_slot);
}
