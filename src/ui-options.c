...
void *my_bsearch(const void *key, const void *base, size_t num,
				 size_t size, comp_fn my_comp);


void *my_bsearch(const void *key, const void *base, size_t num,
				 size_t size, int (*my_comp) (const void *, const void *))
{
...
static void ego_menu(const char *unused, int also_unused)
{
	...
	if (sort_tvals) {
		...
		qsort(raw_tvals, NUM_RAW_TVALS, sizeof(raw_tvals[0]),
			  tval_comp_func);
	}
