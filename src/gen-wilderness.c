					   ...
					   edge_effect * first)
{
	...
	for (j = 0; j < 50; j++) {
		...
		made_plat =
			generate_starburst_room(y0 + y - b, x0 + x - a, y0 + y + b,
	...
	while (form_grids < randint0(20)) {
		...
		x = randint0(CHUNK_WID - 1) + 1;
		form_grids +=
			make_formation(y, x, FEAT_TREE, FEAT_TREE2, form_feats, 3);
	}
	...
	while (form_grids < randint0(30)) {
		...
		x = randint0(CHUNK_WID - 1) + 1;
		form_grids +=
			make_formation(y, x, FEAT_TREE, FEAT_TREE2, ponds, 10);
	}
...
extern void plain_gen_old(void)
{
	...
	while (form_grids < 300) {
		...
		x = randint0(ARENA_WID - 1) + 1;
		form_grids +=
			make_formation(y, x, FEAT_GRASS, FEAT_GRASS, ponds, 10);
	}
...
extern void valley_gen_old(void)
{
	...
	for (i = 0; i < num; i++)
		path_x[i] =
			1 + randint0(ARENA_WID / num - 2) + i * ARENA_WID / num;
