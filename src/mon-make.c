...
/**
 * Deletes all the monsters when the player game is being closed.
 *
 * This is an efficient method of simulating multiple calls to the
 * "delete_monster()" function, with no visual effects.
 */
						   ...
						   struct monster_group_info group_info, uint8_t origin)
{
	...
	/* Set the group info */
	mon->group_info.index = index ? index : monster_group_index_new();
	...
	for (r = 1; r < z_info->r_max; r++) {
		...
		if ((race->base == race1->base) &&
			...
			for (i = start; i < 8 + start; i++) {
				...
				if (place_new_monster_one(c, try, race1, sleep, true,
										  group_info, origin)) {
					/* Success */
					break;
				}
								 ...
								 uint8_t origin)
{
	...
	for (n = 0; (n < loc_num) && (loc_num < escort_size); n++) {
		...
		for (i = start; (i < 8) && (loc_num < escort_size); i++) {
			...
			/* Attempt to place another monster */
			if (place_new_monster_one(c, try, escort_race, sleep, false,
									  group_info, origin)) {
