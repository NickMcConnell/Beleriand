...
static void look_mon_desc(char *buf, size_t max, int m_idx)
{
	...
	if (m_ptr->hp >= m_ptr->maxhp) {
		...
		if (perc >= 60)
			my_strcpy(buf,
					  (living ? "somewhat wounded" : "somewhat damaged"),
					  max);
		else if (perc >= 25)
			my_strcpy(buf, (living ? "wounded" : "damaged"), max);
		else if (perc >= 10)
			my_strcpy(buf, (living ? "badly wounded" : "badly damaged"),
					  max);
		else
			my_strcpy(buf, (living ? "almost dead" : "almost destroyed"),
					  max);
	}
...
void coords_desc(char *buf, int size, int y, int x)
{
	...
	strnfmt(buf, size, "%d %s, %d %s", ABS(y - py), north_or_south,
			ABS(x - px), east_or_west);
}
...
static ui_event target_set_interactive_aux(int y, int x, int mode)
{
	...
	while (1) {
					...
					/* Normal */
						...
						if (p_ptr->wizard) {
							strnfmt(out_val, sizeof(out_val),
									"%s%s%s%s (%s), %s (%d:%d).", s1, s2,
									s3, m_name, buf, coords, y, x);
						} else {
							strnfmt(out_val, sizeof(out_val),
		...
		/* Assume not floored */
		floor_num =
			scan_floor(floor_list, N_ELEMENTS(floor_list), y, x, 0x02);
			...
			if (floor_num > 1)
				while (1) {
					...
					if (p_ptr->wizard) {
						...
						strnfmt(out_val, sizeof(out_val),
								"%s%s%sa pile of %d objects, %s.", s1, s2,
								s3, floor_num, coords);
					}
 ...
 * From NPPangband
 */
static int draw_path(u16b path_n, u16b * path_g, wchar_t * c, int *a,
					 int y1, int x1)
{
	int i;
	bool on_screen;
...
bool target_set_interactive(int mode, int x, int y)
{
	...
	while (!done) {
		...
		if (flag && point_set_size(targets)) {
			...
			/* Remove the path */
			if (path_drawn)
				load_path(path_n, path_g, path_char, path_attr);
...
{
	return target_okay() && panel_contains(target_y, target_x) &&
