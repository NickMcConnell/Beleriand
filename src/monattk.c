...
static void make_request(int m_idx)
{
	...
	if (requested_slot) {
	    ...
	    offer_price =
		(k_ptr->cost * requested_number) + ((5 + randint1(5)) * 100L);
...
bool make_attack_normal(monster_type * m_ptr, int y, int x)
{
	...
	if (!effect || check_hit(power, rlev, terrain_bonus, m_idx)) {
		...
		if (p_ptr->state.free_act) {
		    ...
		    if (p_ptr->timed[TMD_PARALYZED]) {
			if (inc_timed
			    (TMD_PARALYZED, 2 + randint1(rlev / 6), TRUE)) {
			    obvious = TRUE;
			}
		    } else {
			if (inc_timed
			    (TMD_PARALYZED, 4 + randint1(rlev / 2), TRUE)) {
			    obvious = TRUE;
			}
		    }
		}
...
static int get_dam(int av_dam, int dice)
{
	...
	dam =
	    damroll(dice,
		    (av_dam * 2 / dice) - 1) + randint0(((av_dam * 2) % dice) +
							1);
		    ...
		    int degrees_of_arc)
{
    ...
    int flg =
	PROJECT_GRID | PROJECT_ITEM | PROJECT_KILL | PROJECT_ARC | PROJECT_PLAY;
...
bool make_attack_ranged(monster_type * m_ptr, int attack)
{
	...
	mon_arc(m_idx, GF_STORM, TRUE,
		((2 * m_ptr->hp / 5) > 600 ? 600 : (2 * m_ptr->hp / 5)), 0,
		(rf_has(r_ptr->flags, RF_POWERFUL) ? 40 : 20));
	break;
    }
	...
	if (rf_has(r_ptr->flags, RF_MORGUL_MAGIC))
	    mon_ball(m_idx, GF_MORGUL_DARK, get_dam(3 * spower, 12), rad);
	else
	...
	if (spower < 20) {
		...
		msg("%s calls up a storm of nether magics.",
			   m_name);
	    rad = 3;
	}
	...
	if (spower < 40) {
		...
		msg("%s invokes a storm of elemental fury.",
			   m_name);
	    rad = 3;
	}
	...
	if ((spower > 50) && (rf_has(r_ptr->flags, RF_UDUN_MAGIC))) {
	    ...
	    mon_arc(m_idx, GF_HELLFIRE, FALSE, get_dam(7 * spower, 8), 6,
		    60);
	}
	...
	if (seen) {
		...
		msg("%s concentrates on %s wounds.", m_name,
			   m_poss);
	    }
	}
	...
	if (spower < 7) {
		...
		msg("%s points at you and curses horribly.",
			   m_name);
	    k = 2;
	} else if (spower < 30) {
	    if (blind)
		...
		msg("%s points at you, incanting terribly.",
			   m_name);
	    k = 3;
	} else if (spower < 55) {
	    if (blind)
	...
	disturb(1, 0);
	if (blind)
	    msg("%s mumbles, and you hear puzzling noises.",
		       m_name);
	else
	...
	if (blind)
	    ...
	    msg("%s magically summons %s %s.", m_name, m_poss,
		       (rf_has(r_ptr->flags, RF_UNIQUE) ? "minions" :
			"kin"));
	...
	if (blind)
	    ...
	    msg("%s magically summons an undead adversary!",
		       m_name);
	for (k = 0; k < 1; k++) {
