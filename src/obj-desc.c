										 ...
										 bool aware)
{
	...
	bool show_flavor = (k_ptr->flavor
						|| is_jewellery(o_ptr)) ? TRUE : FALSE;
...
static bool obj_desc_show_to_hit(const object_type * o_ptr)
{
	...
	if (o_ptr->to_h
		&& ((if_has(o_ptr->id_other, IF_TO_H)) || k_ptr->to_h.base))
		return TRUE;

	if (is_jewellery(o_ptr) && o_ptr->to_d
		&& if_has(o_ptr->id_other, IF_TO_D))
		return TRUE;
...
static bool obj_desc_show_to_dam(const object_type * o_ptr)
{
	...
	if (o_ptr->to_d
		&& ((if_has(o_ptr->id_other, IF_TO_D)) || k_ptr->to_d.base))
		return TRUE;

	if (is_jewellery(o_ptr) && o_ptr->to_h
		&& if_has(o_ptr->id_other, IF_TO_H))
		return TRUE;
...
static size_t obj_desc_chest(const object_type * o_ptr, char *buf,
...
static size_t obj_desc_combat(const object_type * o_ptr, char *buf,
							  size_t max, size_t end, bool spoil)
{
...
static size_t obj_desc_light(const object_type * o_ptr, char *buf,
							 size_t max, size_t end)
{
...
static size_t obj_desc_pval(const object_type * o_ptr, char *buf,
							size_t max, size_t end)
{
	int i;
	int num_bonus = 0;
...
static size_t obj_desc_charges(const object_type * o_ptr, char *buf,
							   size_t max, size_t end)
{
	object_kind *k_ptr = &k_info[o_ptr->k_idx];
...
static size_t obj_desc_inscrip(const object_type * o_ptr, char *buf,
							   size_t max, size_t end)
{
