...
static errr finish_parse_region(struct parser *p)
{
	...
	for (y = 0; y < MAX_Y_REGION; y++) {
		...
		for (x = 0; x < MAX_X_REGION; x++) {
			...
			square_miles[y][x].map_square.number = x / 49 + 1;
			square_miles[y][x].map_square_grid.y = y % 49;
			square_miles[y][x].map_square_grid.x = x % 49;
		}
	}
		...
		for (y = 0; y < region->height; y++) {
			for (x = 0; x < region->width; x++) {
				...
				if (terrain != ' ') {
					...
					square_miles[y + y_start][x + x_start].region = region;
				}
			}
		}
	}
...
static enum parser_error parse_river_source(struct parser *p)
{
	...
	char letter = parser_getchar(p, "letter");
...
static enum parser_error parse_p_race_difficulty(struct parser *p) {
	struct player_race *r = parser_priv(p);
	if (!r)
		return PARSE_ERROR_MISSING_RECORD_HEADER;
...
static enum parser_error parse_p_race_start_lev(struct parser *p) {
	struct player_race *r = parser_priv(p);
	if (!r)
		return PARSE_ERROR_MISSING_RECORD_HEADER;
	r->start_lev = parser_getint(p, "start");
	return PARSE_ERROR_NONE;
}
