...
struct parser *init_parse_region(void)
{
	...
	parser_reg(p,
			   "I uint danger uint height uint width uint y_offset uint x_offset",
			   parse_region_i);
...
struct parser *init_parse_l(void)
{
	...
	parser_reg(p, "I uint map_y uint map_x uint height uint width",
			   parse_l_i);
...
bool init_angband(void)
{
	...
	event_signal_string(EVENT_INITSTATUS,
						"Initializing arrays... (regions)");
	if (run_parser(&region_parser))
		quit("Cannot initialize regions");
	...
	event_signal_string(EVENT_INITSTATUS,
						"Initializing arrays... (landmarks)");
	if (run_parser(&l_parser))
		quit("Cannot initialize landmarks");
