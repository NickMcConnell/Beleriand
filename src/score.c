...
{
  return (p_ptr->max_exp + (100 * max_depth()));
} */
	...
	for (k = from, j = from, place = k + 1; k < count; k += 5) {
		...
		for (n = 0; j < count && n < 5; place++, j++, n++) {
			...
			strnfmt(out_val, sizeof(out_val),
					...
					p_info[pr].name, c_info[pc].name, clev);

			/* Append a "maximum level" */
			if (mlev > clev)
				my_strcat(out_val, format(" (Max %d)", mlev),
						  sizeof(out_val));
			...
			/* Died where? */
			strnfmt(out_val, sizeof(out_val), "Killed by %s in %s",
					score->how, cdun);

			/* Append a "maximum level" */
			my_strcat(out_val, format(" (Max danger %d)", mdun),
					  sizeof(out_val));
			...
			strnfmt(out_val, sizeof(out_val),
					"(User %s, Date %s, Gold %s, Turn %s).", user, when,
					gold, aged);
						...
						time_t * death_time)
{
	...
	strnfmt(entry->pts, sizeof(entry->pts), "%9lu", (long) p_ptr->score);
	...
	strnfmt(entry->sex, sizeof(entry->sex), "%c",
			(p_ptr->psex ? 'm' : 'f'));
...
void enter_score(time_t * death_time)
{
	...
	/* Hack -- Interupted */
			 ...
			 && streq(p_ptr->died_from, "Interrupting")) {
