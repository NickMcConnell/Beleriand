...
bool recharge(int power)
{
	...
	if (fail) {
	  ...
	  /*** Apply draining and destruction. ***/
			...
			if (fail_type == 1) {
				if (o_ptr->tval == TV_ROD) {
					msg("The recharge backfires, draining the rod further!");
...
void tap_magical_energy(void)
{
	...
	if (o_ptr->tval == TV_ROD) {
		...
		if (o_ptr->pval)
			energy =
				(energy * (o_ptr->pval - o_ptr->timeout)) / o_ptr->pval;
		item_name = "rod";
	}
	if (o_ptr->tval == TV_STAFF) {
...
{
	return (fire_meteor
			(-1, GF_NATURE, p_ptr->py, p_ptr->px, dam, 5, FALSE));
}
...
bool aggravate_monsters(int who, bool the_entire_level)
{
	...
	for (i = 1; i < m_max; i++) {
		...
		/* Standard aggravation */
			...
			if (m_ptr->cdis <
				(p_ptr->themed_level ? MAX_SIGHT : MAX_SIGHT * 2)) {
				/* Wake up */
				if (m_ptr->csleep) {
					/* Wake up */
					m_ptr->csleep = 0;
...
bool tremor(void)
{
	...
	while (!valid_grid) {
		...
		if (cave_empty_bold(ny, nx)
			&& !cave_has(cave_info[ny][nx], CAVE_ICKY)
 ...
 * radius 0. -LM-
 */
bool fire_sphere(int typ, int dir, int dam, int rad,
