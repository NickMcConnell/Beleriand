		...
		if (o_ptr->tval == TV_AMULET)
						...
						while (potential >
							   MIN(2000, (2 * initial_potential / 3)))
						 {
							...
							get_quality(TRUE, SUST_WIS, 0, o_ptr);
								...
								/* Maybe an activation */ 
								if (randint1(5) == 1)
								get_activation(TRUE, EF_RAND_BLESS,
												o_ptr);
			...
			case EGO_AMULET_MAGIC_MAST:
					...
					/* min potential 105 */ 
				{
						...
						/* Bonus to DEX */ 
						if (randint1(4) != 1)
						get_quality(TRUE, ADD_DEX, randint1(bonus),
									 o_ptr);
			...
			case EGO_AMULET_TRICKERY:
					...
					/* min potential 1650 */ 
				{
					...
					get_quality(TRUE, ADD_DEX, 2 + randint0(bonus),
								 o_ptr);
					get_quality(TRUE, SUST_DEX, 0, o_ptr);
					get_quality(TRUE, STEALTH, 2 + randint0(bonus),
								 o_ptr);
			...
			case EGO_RING_COMBAT:
					...
					/* min potential 580 */ 
				{
					...
					if (randint1(temp) > (initial_potential / 6))
						get_quality(TRUE, ADD_STR, randint1(bonus),
									 o_ptr);
					if (randint1(temp) > (initial_potential / 6))
						get_quality(TRUE, ADD_DEX, randint1(bonus),
									 o_ptr);
			...
			case EGO_RING_HINDRANCE:
				...
				ADD_CHR
						...
						if (potential - initial_potential >
							initial_potential)
						potential = 0;
			...
			case EGO_RING_SPEED:
					...
					/* min potential  */ 
				{
					...
					int max_value = 14;
						...
						if (randint1(2) == 1)
						 {
						get_quality(TRUE, SPEED, 1 + max_value / 2,
									 o_ptr);
							...
							get_quality(TRUE, ADD_DEADLINESS, temp,
										 o_ptr);
							get_quality(TRUE, ADD_SKILL, temp, o_ptr);
							}
