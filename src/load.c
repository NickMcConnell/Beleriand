...
byte max_p_res = 0, a_max = 0, max_p_bonus = 0, max_p_slay =
	0, max_p_brand = 0;
...
static int rd_item(object_type * o_ptr)
{
	...
	for (i = 0; i < max_p_brand; i++) {
		...
		if (tmp < 129)
			...
			o_ptr->multiple_brand[i] = (int) (tmp - 256);
	}
...
int rd_artifacts(void)
{
	...
	for (i = art_min_random; i < total_artifacts; i++) {
		...
		for (k = 0; k < a_max; k++) {
			rd_byte(&tmp8u);
			if (tmp8u < 129)
				a_info[i].bonus_stat[k] = (int) tmp8u;
			else
				a_info[i].bonus_stat[k] = (int) (tmp8u - 256);
		}
		for (k = 0; k < max_p_bonus; k++) {
			rd_byte(&tmp8u);
			if (tmp8u < 129)
				a_info[i].bonus_other[k] = (int) tmp8u;
			else
				a_info[i].bonus_other[k] = (int) (tmp8u - 256);
		}
...
int rd_history(void)
{
	...
	for (i = 0; i < tmp32u; i++) {
		...
		history_add_full(type, art_name, z_pos, y_pos, x_pos, clev, turn,
						 text);
	}
...
int rd_chunks(void)
{
	...
	for (j = 0; j < chunk_max; j++) {
		...
		chunk->o_list
			=
			(object_type *) mem_zalloc(z_info->o_max *
									   sizeof(object_type));
		chunk->m_list =
			(monster_type *) mem_zalloc(z_info->m_max *
										sizeof(monster_type));
		chunk->trap_list =
...
int rd_locations(void)
{
	...
	for (i = 0; i < gen_loc_cnt; i++) {
		...
		if (((i % GEN_LOC_INCR) == 0) && (i > 0)) {
			...
			gen_loc_list =
				mem_realloc(gen_loc_list, gen_loc_max * sizeof(gen_loc));
		}
